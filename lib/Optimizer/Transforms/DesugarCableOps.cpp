...
public:
  ...
  LogicalResult matchAndRewrite(quake::AttachWireOp attach,
    ...
    auto split =
        rewriter.create<quake::SplitCableOp>(loc, wiresTy, attach.getCable());
...
public:
  ...
  LogicalResult matchAndRewrite(quake::DetachWireOp detach,
    ...
    SmallVector<Type> wiresTy(size, quake::WireType::get(ctx));
    auto split =
        rewriter.create<quake::SplitCableOp>(loc, wiresTy, detach.getCable());
    ...
    Value bundle = rewriter.create<quake::BundleCableOp>(
        loc, detach.getResult(1).getType(), ValueRange{args});
