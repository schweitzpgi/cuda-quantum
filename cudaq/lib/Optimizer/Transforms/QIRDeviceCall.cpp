...
namespace cudaq::opt {
#define GEN_PASS_DEF_QIRDEVICECALL
#include "cudaq/Optimizer/Transforms/Passes.h.inc"
} // namespace cudaq::opt

#define DEBUG_TYPE "qir-device-call"

using namespace mlir;

namespace {

// QIR-vendor style passthrough lowering: encode a stable per-callback id
// (via an MD5 hash of the callee name) as a `passthrough` function attribute
// on the callee and replace the device_call with a direct call, letting a
// vendor's runtime resolve the callback out of band.
class QIRDeviceCallPat : public OpRewritePattern<cudaq::cc::DeviceCallOp> {
  bool insertTrapImplementation;

public:
  using OpRewritePattern::OpRewritePattern;

  QIRDeviceCallPat(MLIRContext *context, bool insertTrapImpl)
...
class QIRDeviceCallPass
    : public cudaq::opt::impl::QIRDeviceCallBase<QIRDeviceCallPass> {
public:
  using QIRDeviceCallBase::QIRDeviceCallBase;
  ...
  void runOnOperation() override {
    ...
    patterns.insert<QIRDeviceCallPat>(ctx, insertTrapImplementation);
