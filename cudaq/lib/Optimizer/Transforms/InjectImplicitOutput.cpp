...
public:
  ...
  void runOnOperation() override {
    ...
    if (!funcOp->hasAttr(cudaq::entryPointAttrName))
      return;
    // If the kernel already contains any cc.log_output ops at the top level,
    // it has output: do not inject implicit output.
    for (Block &block : funcOp.getBody())
      for (Operation &op : block)
        if (isa<cudaq::cc::LogOutputOp, cudaq::quake::DeallocOp>(op)) {
          LLVM_DEBUG({
            if (isa<cudaq::cc::LogOutputOp>(op))
    ...
    // λ to create a cc.log_output marked as compiler-generated.
    auto emitLogOutput = [&](OpBuilder &b, Location l, Value v) {
      auto op = cudaq::cc::LogOutputOp::create(b, l, ValueRange{v});
    ...
    // 3. Emit cc.log_output ops in declaration order into the new exit block.
